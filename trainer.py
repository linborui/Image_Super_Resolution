...
class TrainerBase:
    ...
    def train(self):
        ...
        for ii in tqdm(range(self.iters_start, self.configs.train.iterations), desc='Training'):
...
# self-supervising based on the distill
class TrainerSSupDistillDifIR(TrainerDistillDifIR):
    def __init__(self, configs):
        super().__init__(configs)
        self.finetune_use_y = configs.train.get("finetune_use_y", False)
    ...
    def training_step(self, data):
        ...
        h_gt, w_gt = data["gt"][0].size()[-2:]
        h_lq, w_lq = data["lq"][0].size()[-2:]
        crop_top = random.randint(0, h_gt - h_lq)
        crop_left = random.randint(0, w_gt - w_lq)

        self.optimizer.zero_grad()
        for jj in tqdm(range(0, current_batchsize, micro_batchsize), desc="Micro Batch", leave=False):
            micro_data = {key:value[jj:jj+micro_batchsize,] for key, value in data.items()}
            last_batch = (jj+micro_batchsize >= current_batchsize)
            tt = torch.randint(
                    0, self.base_diffusion.num_timesteps,
            ...
            model_kwargs_down={'lq':micro_data['lq'],} if self.configs.model.params.cond_lq else None
            ...
            compute_losses_downsample = functools.partial(
                ...
                tt,
                first_stage_model=self.autoencoder,
                model_kwargs=model_kwargs_down,
                noise=noise,
                distill_ddpm=self.distill_ddpm,
                uncertainty_hyper=self.uncertainty_hyper,
                uncertainty_num_aux=self.uncertainty_num_aux,
            ...
            compute_losses_upsample = functools.partial(
                ...
                self.model,
                self.teacher_model,
                micro_gt_crop, # image range 0-1
                tt,
                first_stage_model=self.autoencoder,
                model_kwargs=model_kwargs_up,
                noise=noise,
                distill_ddpm=self.distill_ddpm,
                learn_xT=self.learn_xT,
                finetune_use_y=self.finetune_use_y,
                loss_in_image_space=self.loss_in_image_space
            )
            ...
            for func_type in losses_func:
                compute_losses = losses_func[func_type]
                if self.configs.train.use_fp16:
                    with amp.autocast():
                        if last_batch or self.num_gpus <= 1:
                            losses, z_t, z0_pred = compute_losses()
                    ...
                    loss.backward()
                for key in losses:
                    losses_all[f'{key}_{func_type}'] = losses[key]

            # make logging
            self.log_step_train(losses_all, tt*0 if not self.use_reflow else tt, micro_data, z_t, z0_pred, last_batch)
