...
class GaussianDiffusion:
    ...
    def training_losses_distill_up(
            self, model, teacher_model, y, t,
            first_stage_model=None,
            model_kwargs=None,
            noise=None, distill_ddpm=False, learn_xT=False, finetune_use_y=False, loss_in_image_space=False
            ):
        """
        Compute training losses for a single timestep.
        ...
        :finetune_use_y: do not use teacher model, instead only use the cropped image and its prediction.
        """
        if model_kwargs is None:
            model_kwargs = {}
        ...
        if finetune_use_y:
            z_t_predict = model(self._scale_input(model_output.detach(), t), t*0, **model_kwargs)
            if loss_in_image_space:
                assert self.model_mean_type == ModelMeanType.START_X
                z_y_predict = self.prior_sample(z_t_predict, -noise)
                z_y_predict_rgb = self.decode_first_stage(z_y_predict, first_stage_model, no_grad=False)
                z_y_predict_rgb_down = F.interpolate(z_y_predict_rgb, scale_factor=1/self.sf, mode='bicubic')
                terms[loss_type+"_y_rgb"] = mean_flat((y - z_y_predict_rgb_down) ** 2 if loss_type=="mse" else (y - z_y_predict_rgb_down).abs())
                terms["loss"] += (terms[loss_type+"_y_rgb"]*finetune_use_y)
            else:
                terms[loss_type+"_y"] = mean_flat((z_t - z_t_predict) ** 2 if loss_type=="mse" else (z_t - z_t_predict).abs())
                terms[loss_type+"_y"] /= self.sf**2
                terms["loss"] += (terms[loss_type+"_y"]*finetune_use_y)
